...
/// path from the config. But in the 'dump_snapfile' binary, we need to construct a Layer
/// struct for a file on disk, without having a page server running, so that we have no
/// config. In that case, we use the Path variant to hold the full path to the file on
/// disk. Because dump_snapfile is only used for debugging, and only loads one file at a
/// time, it's OK to leak the path by storing it in 'static lifetime.
pub enum PathOrConf {
    Path(&'static PathBuf),
    Conf(&'static PageServerConf),
}
